...
int main(int argc, char** argv) {
    ...
    //FILE fp=fopen("Solution.txt","a+w");

    if (N > 3) {
        printf("#Solutions\t #FitNodes\t #NonFitNodes\t Coordinates\n");
        ...
        //nq = start(nq, N);
        nq.col[0] = rank;
        printf("Process:%d out of %d processes on %s\n", rank, numprocs, processor_name);
    ...
    printf("CPU time used to execute process %d on %s is %f seconds\n", rank, processor_name, cpu_time_used);
    return (EXIT_SUCCESS);
